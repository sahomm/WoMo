...
void setup() {
  ...
  tft.print("Verb.Aktuell: ");
  tft.setCursor(10,pos_amphr_bat_y);
  tft.print("Verbrauch Ah: ");
  tft.setCursor(10,pos_watthr_bat_y);
  tft.print("Verbrauch Wh: ");
...
float readAmpBat(int mitteln) {
  ...
  float sensdiff=1000.0;         // sense nimmt mit ca. sensdiff/V Vcc ab.
...
float readAmpSolar(int mitteln){
  ...
  for(i=0;i< mitteln;i++) {
    vcc = (float) 3300 / analogRead(5) * 1024.0;    // Versorgungsspannung ermitteln
    //vsensor = (((long)analogRead(A2) * 5000 / 1024) - 500 ) * 1000 / 133; // Messwert auslesen
    vsensor = (((long)analogRead(A2) * vcc / 1024) - 500 ) * 1000 / 133; // Messwert auslesen
