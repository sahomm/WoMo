...
float readAmpBat(int mitteln) {
  ...
  //Serial.println();
  if(debugPWR){Serial.print("AmpBat: ");}
  if(debugPWR){Serial.print(vcc,5);}
  if(debugPWR){Serial.print(" ");}
  if(debugPWR){Serial.print(analogRead(pin_amp_bat));}
  if(debugPWR){Serial.print(" ");}
  if(debugPWR){Serial.print(ampmittel/mitteln);}
  if(debugPWR){Serial.println(" A");}
...
float readAmpSolar(int mitteln){
  ...
  for(i=0;i< mitteln;i++) {
    vcc = (float) 3300 / analogRead(5) * 102.0;    // Versorgungsspannung ermitteln
  ...
  if(debugPWR){Serial.print("AmpSolar: ");}
  if(debugPWR){Serial.print(vcc);}
  if(debugPWR){Serial.print(" ");}
  if(debugPWR){Serial.print(analogRead(pin_amp_solar));}
  if(debugPWR){Serial.print(" ");}
  if(debugPWR){Serial.print(ampmittel/mitteln);}
  if(debugPWR){Serial.println(" A");}
...
float readVoltBat(int mitteln){
  ...
  if(debugPWR){Serial.println("_____________________________________");}   
  if(debugPWR){Serial.print("Spannung: ");}  
  if(debugPWR){Serial.print(vcc,5);}
  if(debugPWR){Serial.print(" ");}
  if(debugPWR){Serial.print(analogRead(pin_volt_bat));}
  if(debugPWR){Serial.print(" ");}
  if(analogRead(0)==1023){
    if(debugPWR){Serial.print("99,99");}
  }else{
  ...
  //if(debugPWR){Serial.print((float)readVccArduino());}
  if(debugPWR){Serial.println("V ");}
